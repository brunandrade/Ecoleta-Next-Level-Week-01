...
const CreatePoint = ()=>{
    ...
    const [initialPosition, setsetInitialPosition] = useState<[number, number]>([0,0]);
...
useEffect(() => {
    navigator.geolocation.getCurrentPosition(position =>{
        ...
        setsetInitialPosition([latitude, longitude]);
    })
},[])
    ...
    useEffect(() => {
        axios.get<IBGEUFResponse[]>('https://servicodados.ibge.gov.br/api/v1/localidades/estados').then(response => {
           // const stateNames = response.data.map(uf => uf.nome);
           // setStates(stateNames);
            const stateInialts = response.data.map(uf => uf.sigla);
